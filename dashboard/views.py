...
def dashboard(request):
    ...
    ).prefetch_related('transfer_items__part').order_by('-transfer_date')[:2]
...
@login_required
@staff_required
def get_maintenance_jobcards(request):
    ...
    ).select_related('scooter', 'technician').order_by('-date_created')
    
    data = []
    for jc in job_cards:
        data.append({
            'job_card_number': jc.job_card_number,
            'scooter': f"{jc.scooter.make} {jc.scooter.model}" if jc.scooter else 'N/A',
            'status': jc.get_status_display(),
            'status_raw': jc.status,
            'priority': jc.get_priority_display(),
            'priority_raw': jc.priority,
            'service_type': jc.get_service_type_display() if jc.service_type else 'N/A',
            'technician': jc.technician.get_full_name() if jc.technician else 'Unassigned',
            'date_created': jc.date_created.strftime('%d %b %Y'),
            'total_cost': float(jc.total_cost) if jc.total_cost else 0,
        })
