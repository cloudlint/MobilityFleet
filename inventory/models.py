...
class Purchase(models.Model):
    ...
    shipping_cost = models.DecimalField(max_digits=10, decimal_places=2, default=0, validators=[MinValueValidator(0)], 
                                   help_text="Shipping cost in currency")
    ...
    def calculate_total_amount(self):
        """Calculate total amount including VAT, shipping and after discount"""
        subtotal = self.subtotal
        discount = self.calculate_discount_amount()
        vat = self.calculate_vat_amount()
        shipping = self.shipping_cost if hasattr(self, 'shipping_cost') else 0
