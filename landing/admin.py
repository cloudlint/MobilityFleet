...
from .models import HomepageVideo, SiteImage, CustomerScooter

admin.site.register(HomepageVideo)


class CustomerScooterAdmin(admin.ModelAdmin):
    list_display = ('user', 'make', 'model', 'year', 'vin', 'color', 'mileage', 'last_service', 'date_added')
    list_filter = ('make', 'year')
    search_fields = ('make', 'model', 'vin', 'user__username')
    readonly_fields = ('date_added',)

admin.site.register(CustomerScooter, CustomerScooterAdmin)


class SiteImageAdmin(admin.ModelAdmin):
    list_display = ('page_location', 'image_preview', 'alt_text', 'is_active', 'updated_at')
    list_editable = ('is_active',)
    readonly_fields = ('image_preview_large', 'updated_at')
    fieldsets = (
        (None, {
            'fields': ('page_location', 'image', 'image_preview_large', 'alt_text', 'is_active'),
        }),
    )
    ...
    def image_preview(self, obj):
        if obj.image:
            return format_html(
                '<img src="{}" style="max-width:120px; max-height:80px; border-radius:5px; border:1px solid #ddd;" />',
                obj.image.url
            )
    ...
    def image_preview_large(self, obj):
        if obj.image:
            return format_html(
                '<img src="{}" style="max-width:400px; border-radius:8px; border:1px solid #ddd;" />',
                obj.image.url
            )
