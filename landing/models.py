...
class CustomerScooter(models.Model):
    user = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name='scooters')
    make = models.CharField(max_length=100)
    model = models.CharField(max_length=100)
    year = models.PositiveIntegerField()
    vin = models.CharField(max_length=100, blank=True, verbose_name="VIN/Serial Number")
    color = models.CharField(max_length=50, blank=True)
    mileage = models.PositiveIntegerField(blank=True, null=True, help_text="Current mileage in km")
    notes = models.TextField(blank=True)
    last_service = models.DateField(blank=True, null=True)
    date_added = models.DateTimeField(auto_now_add=True)

    class Meta:
        verbose_name = "Customer Scooter"
        verbose_name_plural = "Customer Scooters"
        ordering = ['-date_added']
...
class SiteImage(models.Model):
    PAGE_CHOICES = [
        ('rent_hero', 'Rent Page – Hero Image'),
        ('restore_hero', 'Restore Page – Hero Image'),
        ('service_hero', 'Service Page – Hero Image'),
    ]
    page_location = models.CharField(max_length=50, choices=PAGE_CHOICES, unique=True)
    ...
    def __str__(self):
        return self.get_page_location_display()
