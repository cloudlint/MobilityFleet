...
def buy(request):
    ...
    # Get active categories for the Browse by Category section
    categories = Category.objects.filter(is_active=True)
...
def _get_site_image(page_key):
    """Helper to fetch a SiteImage by page_location key"""
    from .models import SiteImage
    try:
        img = SiteImage.objects.get(page_location=page_key, is_active=True)
        return img
...
def rent(request):
    ...
    context = {
        ...
        'hero_image': _get_site_image('rent_hero'),
    }
...
def restore(request):
    ...
    from .models import RestorationGallery
    
    restoration_gallery = RestorationGallery.objects.filter(is_active=True, is_featured=True).order_by('-date_added')[:4]
    
    context = {
        'restoration_gallery': restoration_gallery,
        'hero_image': _get_site_image('restore_hero'),
    }
    
    return render(request, 'landing/restore.html', context)

def service(request):
    """Service and maintenance page"""
    context = {
        'hero_image': _get_site_image('service_hero'),
    }
    return render(request, 'landing/service_new.html', context)
...
def account(request):
    ...
    from .models import CustomerScooter
    user_scooters = CustomerScooter.objects.filter(user=request.user)

    context = {
        'user_scooters': user_scooters,
        'orders': user_orders,
        'rentals': [],
        'services': [],
        'favorites': []
...
def restoration_gallery(request):
    """Restoration gallery page showing all restoration projects"""
    # Import here to avoid circular import
    from .models import RestorationGallery
    
    # Get all active restoration gallery items
    restoration_gallery = RestorationGallery.objects.filter(is_active=True).order_by('-date_added')
    
    context = {
        'restoration_gallery': restoration_gallery
    }
    
    return render(request, 'landing/restoration_gallery.html', context)

def password_reset(request):
    """Handle password reset requests — generates token and sends email"""
    if request.method == 'POST':
        ...
        try:
            ...
            subject = "Password Reset - ScootDr"
            html_message = render_to_string('landing/email/password_reset_email.html', {
                'user': user,
                'reset_url': reset_url,
            })
...
def password_reset_confirm(request, uidb64, token):
    """Handle password reset confirmation — validates token and sets new password"""
    from django.contrib.auth.tokens import default_token_generator
    ...
    if user is not None and default_token_generator.check_token(user, token):
        if request.method == 'POST':
            ...
            if new_password != confirm_password:
                ...
                return render(request, 'landing/password_reset_confirm.html', {'valid_link': True})
        ...
        return render(request, 'landing/password_reset_confirm.html', {'valid_link': False})
...
def update_profile(request):
    """Handle profile update from account page"""
    if request.method == 'POST':
        user = request.user
...
def change_password(request):
    """Handle password change from account page"""
    if request.method == 'POST':
...
def add_scooter(request):
    """Handle adding a customer scooter"""
    if request.method == 'POST':
        from .models import CustomerScooter
        ...
        if not make or not model or not year:
            messages.error(request, "Make, model, and year are required.")
            return redirect('landing:account')
        ...
        CustomerScooter.objects.create(
            user=request.user,
            make=make,
            model=model,
            year=year_int,
            ...
            mileage=int(request.POST.get('mileage') or 0) or None,
            notes=request.POST.get('notes', '').strip(),
        )
        messages.success(request, f"Your {year} {make} {model} has been added.")
    return redirect('landing:account')


@login_required
def remove_scooter(request, scooter_id):
    """Handle removing a customer scooter"""
    if request.method == 'POST':
        from .models import CustomerScooter
        try:
            scooter = CustomerScooter.objects.get(id=scooter_id, user=request.user)
            name = f"{scooter.year} {scooter.make} {scooter.model}"
            scooter.delete()
            messages.success(request, f"{name} has been removed from your account.")
        except CustomerScooter.DoesNotExist:
            messages.error(request, "Scooter not found.")
    return redirect('landing:account')