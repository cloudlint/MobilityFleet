...
def job_card_list(request):
    ...
    # Show all job cards without pagination
    job_cards = job_cards_queryset
        ...
        if form.is_valid():
            ...
            if formset.is_valid():
                ...
                if parts_valid:
                    ...
                    # If parts validation failed, keep the job card but don't allocate parts
                    # Restore original scooter status since no parts were processed
                    scooter.status = job_card.previous_scooter_status
                    scooter.save()
                    messages.warning(request, f'Job Card {job_card.job_card_number} was created successfully, but parts could not be allocated due to insufficient stock. You can edit the job card later to add parts.')
                    return redirect('service:job_card_list')
            else:
                # If formset is invalid, keep the job_card but show error
                # Restore original scooter status since no parts were processed
                scooter.status = job_card.previous_scooter_status
                scooter.save()
                messages.warning(request, f'Job Card {job_card.job_card_number} was created successfully, but there was an issue with the parts data. You can edit the job card later to add parts.')
                return redirect('service:job_card_list')
        else:
...
def job_card_detail(request, pk):
    ...
    if 'print' in request.GET:
        # Get technician and service type from GET parameters
        selected_technician = request.GET.get('technician', job_card.technician.get_full_name() or job_card.technician.username)
        selected_service_type = request.GET.get('service_type', job_card.service_type)
...
@login_required
def format_print_options(request, pk):
    """View to show format print options for technician and service type selection"""
    ...
    # Get service type choices
    service_types = JobCard.SERVICE_TYPE_CHOICES
    
    context = {
        'job_card': job_card,
        'technicians': technicians,
        'service_types': service_types,
        'current_technician': job_card.technician,
        'current_service_type': job_card.service_type
    }
    
    return render(request, 'service/format_print_options.html', context)
