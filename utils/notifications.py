...
def send_inventory_alert_email(alert, recipients=None):
    ...
    logger.warning(f"INVENTORY ALERT: {alert.title} - {alert.description} - Severity: {alert.get_severity_display()}")
        ...
        try:
            ...
            for recipient in recipients:
                ...
                if response.status_code not in [200, 201, 202]:
                    logger.error(f"SendGrid error sending inventory alert: {response.status_code}")
                    return False
            
            # Mark alert as emailed
            alert.email_sent = True
            alert.save(update_fields=['email_sent'])
            logger.info(f"Inventory alert email sent successfully via SendGrid for alert {alert.id}")
            return True
            
        except Exception as e:
            logger.error(f"SendGrid error while sending inventory alert: {e}", exc_info=True)
    ...
    try:
        ...
        alert.email_sent = True
        alert.save(update_fields=['email_sent'])
        logger.info(f"Inventory alert email sent successfully via Django mail for alert {alert.id}")
        return True
        
    except Exception as e:
        logger.error(f"Email error while sending inventory alert: {e}", exc_info=True)
        
        # For development without email configuration
        # Mark as emailed anyway to avoid repeated attempts
        alert.email_sent = True
        alert.save(update_fields=['email_sent'])
        logger.warning(f"Inventory alert marked as emailed despite email error for alert {alert.id}")
...
def send_sms_notification(phone_number, message):
    ...
    if not USE_TWILIO:
        ...
        logger.debug(f"Twilio not configured, SMS notification skipped for {phone_number}")
        return True
    ...
    try:
        ...
        logger.error(f"Twilio SMS error while sending to {phone_number}: {e}", exc_info=True)
...
def send_critical_inventory_alert(alert, phone_numbers=None):
    ...
    logger.critical(f"CRITICAL INVENTORY ALERT: {alert.title} - {alert.description}")
